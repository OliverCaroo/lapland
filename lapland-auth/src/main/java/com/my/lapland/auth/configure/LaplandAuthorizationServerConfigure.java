 ...
 * @author yu.mei
 * @Description 认证服务器配置
 * @create 2020/3/31 0:09
 */
@Configuration
@EnableAuthorizationServer
public class LaplandAuthorizationServerConfigure extends AuthorizationServerConfigurerAdapter {

    @Autowired
    private AuthenticationManager authenticationManager;
    @Autowired
    private LaplandUserDetailService userDetailService;
    @Autowired
...
//                .scopes("all");
        LaplandClientsProperties[] clientsArray = authProperties.getClients();
        InMemoryClientDetailsServiceBuilder builder = clients.inMemory();
        if (ArrayUtils.isNotEmpty(clientsArray)) {
            for (LaplandClientsProperties client : clientsArray) {
