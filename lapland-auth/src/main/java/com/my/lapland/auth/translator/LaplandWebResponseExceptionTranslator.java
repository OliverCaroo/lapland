package com.my.lapland.auth.translator;
 ...
 * @author yu.mei
 * @Description 异常翻译
 * @create 2020/5/24 19:51
 */
@Slf4j
@Component
@SuppressWarnings("all")
public class LaplandWebResponseExceptionTranslator implements WebResponseExceptionTranslator {

    @Override
    public ResponseEntity<?> translate(Exception e) {
        ResponseEntity.BodyBuilder status = ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR);
        LaplandResponse response = new LaplandResponse();
        String message = "认证失败";
        log.error(message, e);
